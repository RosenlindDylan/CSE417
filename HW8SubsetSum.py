# intialize Opt matrix with first column of 1's first row of 0's 
# and Opt[0,0] = 1
# for i from 1 to n+1
# 	for j from 0 to K + 1
# 		if S[i-1] <= j
# 			Opt[i][j] = Opt[i-1][j] + Opt[i-1][j - S[i-1]]
# 		else:
# 			Opt[i][j] =Opt[i-1][j]
# return Opt[n][K]
...
print(SubsetSum(electoralVotes2024, len(electoralVotes2024), 269))